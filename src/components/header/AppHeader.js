...
const AppHeader = ({ bussinessName, Bussiness_location, bussinessimg, children, ...props }) => {
  ...
  useEffect(() => {
    const updatePageStatus = () => {
      setIsHomePage(location.pathname === "/homepage" || location.pathname === "/BussinessPage");
    };
  ...
  return (
    <div>
      <Navbar className={`app-header ${isHomePage ? 'home-page' : ''}`} expand="md" fixed="top">
        <NavbarBrand onClick={() => handleNavigate('/homepage')}>
          {location.pathname === `/bussinessPage/${id}` ? (
            <div style={{ display: "flex", justifyContent: "center", position: "relative", left: "95px" }}>
              <img src={bussinessimg} alt="mainLogo" className="bussiness-logo" />
              <div className='bussiness_txt'>
                <h6> <span style={{ marginRight: "7px" }}><FaRegClock /></span>{bussinessName}</h6>
                <h6> <span style={{ marginRight: "7px" }}><IoLocation /></span>{Bussiness_location}</h6>
              </div>
            </div>
          ) : (
        ...
        {children}
        <Nav className="ml-auto" navbar style={{ position: 'relative', right: '75px', top: (isHomePage && window.innerWidth <= 767) ? '-45px' : '0px' }}>

          <NavItem>
            <NavLink onClick={() => handleNavigate("/cart")}>
              <FaCartArrowDown className="icon" /> <span className="app-text">Cart<span className="cart__badge">{totalQuantity >= 0 ? totalQuantity : 0}</span></span>
            </NavLink>
          </NavItem>

          <NavItem>
            <NavLink onClick={() => handleNavigate("/Notification")}>
              <FaBell className="icon" /> <span className="app-text">Notifications</span>
            </NavLink>
          </NavItem>
