...
const ForgotPassword = ({ show, handleClose }) => {
    ...
    return (
        <Helmet title="Forgot-Password">
                ...
                <Row>
                    ...
                    <Col lg="5" md="6" className="d-flex align-items-center justify-content-center">
                        <div style={{ padding: '20px', width: '80%' }}>
                            <span style={{ fontSize: "1.9rem", position: "relative", bottom: "15px", cursor: "pointer" }} onClick={backHandler}>
                                <IoArrowBackCircleOutline />
                            </span>
                            <h3>Forgot Password</h3>
                            <p style={{ fontSize: "13px", fontFamily: "Poppins" }}>Enter your registered email address to receive a link via email. </p>
                            <form onSubmit={submitHandler}>
                                <div className="mb-4 input-group">
                                    <input type="text" className="form-control shadow-none" id="email" placeholder='Email' value={userEmail} onChange={(e) => setUserEmail(e.target.value)} />
                                    <span className="input-group-text"><FaEnvelope /></span>
                                </div>
                                <div>
                                    <span className='error-mess'> {error && <div style={{ color: '#FF0000', position: "relative", bottom: "10px" }}>{error}</div>} </span>
            ...
            <Modal show={isDialog} onHide={handleClose} centered style={{ borderRadius: '8px' }}>
                <Modal.Header>
                ...
                <Modal.Body className="text-center" style={{ padding: "20px" }}>
                    ...
                    <h6 className='subTxt'>A link has been sent to your email <span style={{ fontWeight: "bold" }}>{userEmail}.</span> </h6>
                    <button onClick={() => setIsDialog(false)} className='close_ok_btn'>Ok</button>
                </Modal.Body>
            </Modal>
