  ...
  const loginHandler = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          "Login API Response: " + JSON.stringify(responseData.token)
        );
        ...
        if (role === "business") {
          const res = await fetch(
            `https://dmecart-38297.botics.co/business/stripe_verification/${id}/`
          );
        ...
        setToastError(
          "Your business is not approved yet, please try again after some time"
        );
      } else {
        setShowErrorToast(true);
        console.error("Login failed. Please check your credentials.");
